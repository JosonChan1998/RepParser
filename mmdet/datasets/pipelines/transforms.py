...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal', flip_map=None):
    ...
    def parsing_flip(self, gt_parsings, direction):
        ...
        for l_r in self.flip_map:
            left = np.where(flipped_parsings == l_r[0])
            right = np.where(flipped_parsings == l_r[1])
            flipped_parsings[left] = l_r[1]
